...
public class JellyComponent extends ActorComponent {
    ...
    static void forwardDiffBezier(double c0, double c1, double c2, double c3, List<Vec2D> points, int count,
            int offset) {
    ...
    public void update(int dirt) {
        ...
        if (_inTarget != null) {
            ...
            if (firstBone == bone && parentBoneJelly != null && parentBoneJelly._outTarget != null) {
                Vec2D translation = parentBoneJelly._outTarget.getWorldTranslation(Vec2D());
                Vec2D localParentOut = Vec2D.transformMat2D(Vec2D(), translation, inverseWorld);
        ...
        if (_outTarget != null) {
            ...
            if (firstBoneJelly != null && firstBoneJelly._inTarget != null) {
                Vec2D translation = firstBoneJelly._inTarget.getWorldTranslation(Vec2D());
                Vec2D worldChildInDir = Vec2D.subtract(Vec2D(), firstBone.getWorldTranslation(Vec2D()), translation);
            ...
            Vec2D.normalize(_outDirection, _outDirection);
            Vec2D scaledOut = Vec2D.scale(Vec2D(), _outDirection, _easeOut * bone.size() * curveConstant);
